...
namespace sb_io {
  ...
  public:
    PrinterConnection(const char * switchbox_hostname,
  ...
  public:
     ...
     *
     * This function will cause the Connection to disconnect from the
     * switchbox. The memory will also have delete called on it.
     *
     * @param key The key to associate with this connection.(or id)
     *
     * @return True if it added sucessfully, False if it didn't.
    ...
    bool spawnPrinterConnection(){
      ...
      if ( connection->getAddress() != 0 ){
        cerr << "Warning: The Printer connection's address is not 0. This"
             << endl
             << "         likely means that the input will not match the output!"
             << endl;
      }
      connection_map.insert(std::pair< int, Connection* >(key, connection));
      if (CMDEBUG)
        cerr << "ConnectionManager::spawnPrinterConnection(): success"
             << endl;
      return true;}
  ...
  public:
    MonitorConnection(const char *switchbox_hostname,
                      const int switchbox_port)
      : Connection(switchbox_hostname,switchbox_port) {};
    virtual void handleAnnounceMessage(SBMessage *msg){
      if (CMDEBUG) cerr << "MonitorConnection::handleAnnounceMessage()"
                        << endl;
      if ( msg->from != msg->to ){
      ...
      // Now that we're done inspecting the message let the connection
      // object do its real work.
  ...
  public:
    ...
    ~RemoteConnectionManager(){
      ...
      if ( remoteClients ){
        ...
        for (it = key_to_remote_address.begin();
             it != key_to_remote_address.end();
             it++) {
    ...
    bool addRemoteConnection(int key, std::string remote_hostname){
      char command[2048];
      if ( !remoteClients ){
        // I am the first, so I need to spawn a switchbox and create a
        // connection to it.
