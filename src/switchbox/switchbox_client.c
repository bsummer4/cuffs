...
SBMessage *
message_with_unset_data (int size, int from, int to, int type)
{
...
SBMessage *
message (int size, int from, int to, int type, char *data)
{
...
SBMessage *
make_group (int from, int group_id, int num_clients, int *clients)
{
...
SBMessage *
remove_group (int from, int group_id)
{
  size_t header_size = SBMESSAGE_HEADER_SIZE;
  size_t admin_header_size = ADMIN_HEADER_SIZE;
...
SBMessage *
string_to_message (int type, int from, int to, char *string)
{
...
char *
message_data (SBMessage * m)
{
  return m->data;
}

int
message_from (SBMessage * m)
{
  return m->from;
}

int
message_to (SBMessage * m)
{
  return m->to;
}

int
message_type (SBMessage * m)
{
  return m->routing_type;
}

void
free_message (SBMessage * m)
{
  free (m);
}

char *
copy_message_string (SBMessage * m)
{
  return strdup (message_data (m));
}

int
announcement_code (SBMessage * m)
{
  return *((int *) m->data);
}
...
bool
switchbox_send (Socket s, SBMessage * m)
{
  if (!m || (unsigned) m->size < (sizeof (int) * 4))
    {
      if (debug)
	printf ("switchbox_send:  bad message.  \n");
      return false;
    }
  if (debug)
