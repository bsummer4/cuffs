...
bool
switchbox_send_string (Socket s,
		       int size, int type, int from, int to, char *string)
{
...
char *
switchbox_receive_string (Socket s, int *size, int *type, int *from, int *to)
{
...
switchbox_send (Socket s, SBMessage * m)
{
  if (!m)
    return false;
  if ((unsigned) m->size < (sizeof (int) * 4))
    return false;
  if (debug)
