...
receive_message (Socket socket)
{
  ...
  if (!success)
    goto receive_message_error;
  return result;
receive_message_error:if (result)
    free (result);
  return NULL;
}

bool
send_string (Socket socket, char *string)
{
...
/** This is basically the same as send_message().  However, the data
    ...
    len(array)).  */
bool
send_array (Socket socket, int length, byte * array)
{
...
bool
write_bytes (byte * output, size_t length, Socket s)
{
  char *pointer = output;
...
bool
read_bytes (byte * input, size_t length, Socket s)
{
  if (!valid_socket (s))
    return false;
  char *pointer = input;
...
bool
valid_socket (Socket socket)
{
  return socket != -1;
}

bool
valid_listener (Listener listener)
{
  return listener != -1;
}
...
Socket
open_connection (const char *hostname, const int port)
{
