...
char *
routing_type_to_string (int routing_type)
{
  switch (routing_type)
  {
  case UNICAST:
...
bool
define_group (int group_id, int num_clients, int *users)
{
  if (debug)
  {
...
bool
switchbox_handle_admin (SBMessage * m)
{
  if (debug)
    printf ("admin message: [");
  if (debug)
    iter (ii, 0, m->size) printf ("%x ", (unsigned) ((char *) m)[ii]);
  if (debug)
    printf ("]\n");
  ...
  size_t clients_size =
    admin_message_size - (sizeof (admin_task_t) + sizeof (int));
  ...
  case DEFINE_GROUP:
    send_error (m->from,
		define_group (gn, num_clients, 
			      clients) ? ADMIN_SUCCESS : ADMIN_FAIL);
    break;
...
bool
switchbox_locking_send (SBMessage * m)
{
...
bool
multicast (SBMessage * m)
{
  if (debug)
    printf ("multicasting to %d\n", m->to);
  int group_id = m->to;
  if (debug && group_id >= MAX_GROUPS)
...
void
announce (int code, int client_id)
{
...
void *
handle_connection (void *client_)
{
...
void
send_hello_message (Socket s, int addr)
{
  SBMessage *msg = malloc (sizeof (int) * 4);
  msg->size = sizeof (int) * 4;
