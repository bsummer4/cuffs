...
namespace vectors {
  ...
  /// the only way I could find to call this function in a method
  /// called 'lround'.
  int _lround(double x) { return lround(x); }

  struct Vector2_dbl : public Vector2 <double> {
    Vector2_dbl() : Vector2 <double> () {}
    Vector2_dbl(double x, double y) : Vector2 <double> (x, y) {}
    Vector2_dbl(const Vector2 <int> &v) : Vector2 <double> (v.x, v.y) {}
    Vector2_dbl(const Vector2 <double> &v) : Vector2 <double> (v.x, v.y) {}
    Vector2 <int> floor() {
